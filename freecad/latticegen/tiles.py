...
class VoronoiTile(BaseTile):
    ...
    @classmethod
    def get_grid_dimensions(cls, step_radius: float):
        # Base it on a hexagonal staggered grid for organic packing
        dx = 1.5 * step_radius
        dy = math.sqrt(3) * step_radius
        return dx, dy, True
    ...
    @staticmethod
    def create_face(base_pos, norm, tan_u, tan_v, radius):
        # Deterministic pseudo-random seed based on coordinate rounding
        # This guarantees the pattern holds steady upon recomputes
        seed_str = f"{round(base_pos.x, 2)}_{round(base_pos.y, 2)}_{round(base_pos.z, 2)}"
        ...
        # Randomize number of sides (5 to 7 sides for organic variation)
        num_sides = 5 + int(rand() * 3)
        base_angle = rand() * math.pi * 2
        
        local_pts = []
        for i in range(num_sides):
            # Distribute angles to ensure the polygon remains strictly star-convex
            slice_angle = 2 * math.pi / num_sides
            angle = base_angle + i * slice_angle + slice_angle * 0.4 * (rand() - 0.5)
            
            # Vary radius to create the signature Voronoi unequal struts
        ...
        try:
            ...
            return HexagonTile.create_face(base_pos, norm, tan_u, tan_v, radius)
