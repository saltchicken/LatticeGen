...
from freecad.latticegen.constants import MIN_DENOM, TOL_RELAXED, TOL_STRICT
...
class CylindricalStrategy(WrapStrategy):
    ...
    def get_mapping(self, u: float, v: float):
        ...
        tan_u = App.Vector(-math.sin(theta), math.cos(theta), 0) * self.wrap_scale
...
class ProjectedCylindricalStretchedStrategy(WrapStrategy):
    ...
    def get_mapping(self, u: float, v: float):
        ...
        scale_u = self.wrap_scale * (r_hit / self.R) if self.R > TOL_STRICT else self.wrap_scale
...
class ProjectedCylindricalUniformStrategy(WrapStrategy):
    ...
    def setup_grid(self, dx: float, dy: float, u_min: float, u_max: float, v_min: float, v_max: float, is_staggered: bool):
        ...
        ray = Part.makeLine(p_center, p_center + App.Vector(self.max_dim * 2, 0, 0))
        intersections = self.target_shape.common(ray)
