...
class WrapStrategy(BaseMappingStrategy):
    ...
    def get_clipping_shape(self, border_size: float):
        ...
        if border_size > 0.0:
            ...
            if z_max > z_min:
                ...
                box_pos = App.Vector(self.Cx - safe_size / 2.0, self.Cy - safe_size / 2.0, z_min)
    ...
    def is_tile_valid(self, test_pts, inclusion_threshold: float) -> bool:
        ...
        if inclusion_threshold >= PERCENT_MAX:
            return all(z_min_orig - TOL_RELAXED <= pt.z <= z_max_orig + TOL_RELAXED for pt in pts_to_check)

        inside_count = sum(1 for p in pts_to_check if z_min_orig - TOL_RELAXED <= p.z <= z_max_orig + TOL_RELAXED)
