...
def _generate_2d_tiles(target_shape, strategy, config: LatticeConfig):
    ...
    for c in range(c_start, c_end):
        for r in range(-1, rows):
            ...
            tile_face, test_pts = tile_generator.create_face(base_pos, norm, tan_u, tan_v, config.tile_radius)
...
def _build_3d_pillars(pattern_faces, strategy, target_shape, config: LatticeConfig):
    ...
    for tile_face, norm in pattern_faces:
        extrude_vec = strategy.get_extrude_vector(norm, config.extrude_depth, z_height)
        ...
        if config.fillet_radius > 0.0 and "Circle" not in config.pattern:
            ...
            if edges_to_fillet:
                try:
                    filleted = pillar.makeFillet(config.fillet_radius, edges_to_fillet)
...
def generate_lattice_shape(target_obj, config: LatticeConfig, target_face=None, return_tool=False):
    ...
    tool_compound = _build_3d_pillars(pattern_faces, strategy, target_shape, config)
